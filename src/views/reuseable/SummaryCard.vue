          ...
          <div v-for="(item, index) in items" v-bind:key="index" class="col">
            <div class="d-flex align-items-center me-2">
              ...
              <div class="symbol symbol-50px me-3">
                <div :class="`symbol-label bg-light-${item.iconColor}`">
                  <span
                    :class="`svg-icon svg-icon-1 svg-icon-${item.iconColor}`"
                  >

                    <inline-svg :src="item.icon||'media/icons/duotune/ecommerce/ecm010.svg'" />
                  </span>
                </div>
              </div>
              <!--end::Symbol-->
              ...
              <!--begin::Title-->
              <div>
                <div class="fs-4 text-dark fw-bolder">{{ item.value }}</div>
                <div class="fs-7 text-muted fw-bold">{{ item.title }}</div>
              </div>
